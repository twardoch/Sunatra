...
if SENTRY_DSN and SENTRY_DSN != "YOUR_DSN_HERE":
    try:
        ...
        sentry_sdk.init(
            ...
            send_default_pii=False,      # never auto-attach user/IP/cookies
            before_send=scrub_event,     # redact token/cookie/secret values
        )
...
class SunatraApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # 1. Set AppUserModelID (Separates icon in Taskbar) — Windows only.
        ...
        try:
            ...
            logo_icon_path = resource_path("assets/SunatraLogoIcon.png")
    ...
    def check_changelog(self):
        ...
        if last_version != current_version:
            messagebox.showinfo(
                ...
                f"🎉 Welcome to Sunatra v{current_version}! 🎉\n\n"
    ...
    def load_window_state(self):
        try:
            if os.path.exists("window_state.json"):
                with open("window_state.json") as f:
                    data = json.load(f)
...
def main():
    """Console/GUI entry point (referenced by pyproject `gui-scripts`)."""
    ...
    # Migrate any legacy data into the Sunatra data dir before the app reads
    # config/manifest (see core.app_meta). Idempotent and non-destructive.
    try:
        migrate_legacy_data()
    except Exception as e:
        print(f"Legacy data migration skipped: {e}")
